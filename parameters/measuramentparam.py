...
class MeasuramentParameters:
    ...
    def config_file2dict(self, file) -> dict:
        config_dict = MeasurantParameter.config_dict_default
...
class MeasurantParameters:
    # FIXME: 第一優先　パラメータをしっかりできるように！
    zig         :bool = False
    app_points  :int  = 12000,
    ret_points  :int  = 12000,
    xlength     :int  = 3.0e-6
    ylength     :int  = 3.0e-6
    xstep       :int  = 20,
    ystep       :int  = 20,
    ...
    def config_file2dict(self, file) -> dict:
        config_dict = ConfigLoader.config_dict_default
        for l in file.readlines():
            l = l[:-1]
            number = re.findall(r"[-+]?\d*\.\d+|\d+", l.strip("\n"))
            if len(l) == 0:
