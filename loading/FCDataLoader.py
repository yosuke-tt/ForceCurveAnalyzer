from .MeasurantParameters import MeasurantParameters
from .FilePathDC import FilePathes
import os
import sys
import warnings
...
class FCDataLoader(FCBaseProcessor):
    ...
    def load_row_fc(
        ...
        assert isinstance(self.map_shape, tuple), "map_shape is boolean"

        if len(fc_row_data) != self.map_shape[0] * self.map_shape[1]:
            am = self.map_shape
            self.map_shape = (int(np.sqrt(len(fc_row_data))), int(np.sqrt(len(fc_row_data))))
            print(f"lenght of fc_row_data {len(fc_row_data)} is not same as map_shape {am}=>{self.map_shape}")
            allow_any_shape = True
            if self.map_shape[0]**2 != len(fc_row_data) or not allow_any_shape:
                print("data shape is not square")
        return fc_row_data

    def direct_zig(self, data, data_length=1):
        numbering = np.arange(len(data), dtype=np.int).reshape(self.map_shape)
        numbering[1::2] = numbering[1::2, ::-1]
        np.savetxt(self.savefile2savepath("nubering"), numbering)
        if data_length > 1:
            data = data.reshape(*self.map_shape, data_length)
            data[1::2] = data[1::2, ::-1]
            return data.reshape(-1, data_length)

        else:
            data = data.reshape(self.map_shape)
            data[1::2] = data[1::2, ::-1]
            return data.reshape(-1, self.map_shape[0] * self.map_shape[1])
    ...
    def get_topo_img(self, zsensor, contact):
        ...
        topo_contact : bool
            コンタクトポイントでのトポグラフィー像
        """
        # topo_trig = np.max(zsensor, axis=1).reshape(self.map_shape)
        self.topo_contact = np.array([z[c] for z, c in zip(zsensor, contact)]).reshape(self.map_shape)
    ...
    def gradient_adjasment(self, data):
        ...
        data_ga:array_like
            傾斜補正したデータ
        """
        data_gra = np.array(data).reshape(self.map_shape) / self.cos_map
    ...
    def split_app_ret(self, data):
        ...
        """
        return data[:, :self.length_of_app], data[:, self.length_of_app:]
