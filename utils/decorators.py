    ...
    def ds_func(func):
        ...
        def data_statistics(self, *args):
            ...
            for i, (d, dn) in enumerate(zip(data, data_name)):
                ...
                try:
                    d = np.array(d)
                    np.save(os.path.join(self.save_path, dn), d)
                    ...
                    if "map_only" in stat_type:
                        plt.imshow(d.reshape(self.map_shape),
                                   vmin=vmin, vmax=vmax, cmap="Greys")
                        plt.colorbar()
                        plt.savefig(os.path.join(self.save_path, dn))
                        plt.close()
                        num_plot -= 1
                    ...
                    if "map" in stat_type:
                        mapple = ax[i].imshow(
                            d.reshape(self.map_shape), vmin=vmin, vmax=vmax, cmap="Greys")
                    ...
                    plt.savefig(os.path.join(self.save_path, dn + "stat"))
                    plt.close()
                except TypeError:
